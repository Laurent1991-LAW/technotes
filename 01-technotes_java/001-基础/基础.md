...
In Java, the most common data types can be categorized into two groups: primitive data types and reference data types. Primitive data types are predefined by the language and are named by a reserved keyword. Here are the most common primitive data types in Java along with their lengths:

1. **byte**: This is an 8-bit signed two's complement integer. Its minimum value is -128 and maximum value is 127.

2. **short**: This is a 16-bit signed two's complement integer. Its minimum value is -32,768 and maximum value is 32,767.

3. **int**: This is a 32-bit signed two's complement integer. Its minimum value is -2,147,483,648 and maximum value is 2,147,483,647.

4. **long**: This is a 64-bit signed two's complement integer. Its minimum value is -9,223,372,036,854,775,808 and maximum value is 9,223,372,036,854,775,807.

5. **float**: This is a single-precision 32-bit IEEE 754 floating point. It is used when the precision is not required.

6. **double**: This is a double-precision 64-bit IEEE 754 floating point. It is used when more precision is required.

7. **boolean**: This type can only take the values true or false.

8. **char**: This is a single 16-bit Unicode character. It has a minimum value of '\u0000' (or 0) and a maximum value of '\uffff' (or 65,535).

<u>Reference data types</u> 引用对象 include classes, interfaces, and arrays. These types are not covered in the length as they are not fixed-size like <u>primitive types</u> 基础对象.



|  Java  | SQL                   | length               |
| :----: | --------------------- | -------------------- |
|  byte  | smallint              | 8-bit / 1 byte       |
| short  | mediumint             | 16-bit / 2 bytes     |
|  int   | int                   | 32-bit / 4 bytes     |
|  long  | bigint                | 64-bit / 8 bytes     |
| float  | float                 |                      |
| double | decimal               |                      |
...
- 未重写的hashCode方法是一个native方法，是底层实现的，通过将**该对象的内部地址转换成一个整数**来实现的，代表一个地址；
...
In Java, the length of a string is limited by the amount of memory available to the program. Internally, strings in Java are represented as instances of the `java.lang.String` class, which stores characters in an array of `char`. The maximum length of an array in Java is determined by the maximum value of an `int`, which is `Integer.MAX_VALUE` (2^31 - 1 or 2,147,483,647).

String底层是char数组, 所以一个string最长可以有 char[`Integer.MAX_VALUE`] 这么长:

- 如果 每一位元素 都是用asc编码, 则每位占内存 1 byte, Memory in bytes = 2,147,483,647 bytes, 约等于2G;
- 如果用utf8编码, 一般字符会用2-4字节, 取平均值3个字节, 则最长的string可以达到6G;

However, in practice, you are unlikely to reach this limit due to memory constraints. Creating a string of such a length would require a <u>significant amount of memory</u>, and you would likely encounter an `OutOfMemoryError` long before reaching the theoretical limit.
...
- 直接使用 双引号声明出来的String对象 会直接存储在常量池中；
- 使用String s1 = new String("xxx")创建出的字符串，会开辟内存空间存储为一个StringObject对象，使用String提供的intern()方法，将在 字符串常量池 中查询 当前字符串看是否存在，若 不存在 则会在堆中生成"xxx"的字符串对象o，将其引用存在常量池中；
...
自JDK7开始，常量池 中不再存放 字符串字面量本身，而是将该对象挪至堆中，常量池存储 对该字符串 的引用

<img src="\images\640.png" alt="640" style="zoom: 67%;" />
...
In Java, exceptions are problems that can occur during the execution of a program. Java categorizes exceptions under two main types: checked exceptions and runtime exceptions.

1. **Checked Exceptions**:
   - Checked exceptions are exceptions that are checked at compile-time. This means that the code must handle these exceptions by either throwing the exception or by catching it and dealing with it in some manner. If a method is capable of causing an exception that it does not handle, it must declare this behavior in its method signature using the `throws` keyword.
   - Examples of checked exceptions include `IOException`, `FileNotFoundException`, and `ClassNotFoundException`.
   - Checked exceptions are subclasses of `java.lang.Exception` but not subclasses of `java.lang.RuntimeException`.

2. **Runtime Exceptions**:
   - Runtime exceptions are not checked at compile-time, but rather, at runtime. These are often referred to as unchecked exceptions.
   - Runtime exceptions can occur due to programming errors such as logic errors or improper use of an API. They are not typically caught by the program since they could represent bugs that should be fixed in the code rather than being handled by the application at runtime.
   - Examples of runtime exceptions include `NullPointerException`, `ArrayIndexOutOfBoundsException`, `ClassCastException`, and `ArithmeticException`.
   - Runtime exceptions are subclasses of `java.lang.RuntimeException`, which itself is a subclass of `java.lang.Exception`.

The distinction between the two lies primarily in how the Java compiler handles them. Checked exceptions must be explicitly handled or declared by the code, while runtime exceptions are optional to handle, and the compiler does not enforce a method declaration with `throws` for runtime exceptions.
...
Classloader : c/c++实现，无法作为对象被引用，负责 /lib目录下 类加载

Extension Classloader : java实现，负责 /lib/ext 目录下类加载

Application Classloader : java实现，负责 classpath 目录下类加载
...
> **Each thread has its own JVM stack**, which is created at the same time as the thread. The JVM stack stores frames 栈帧 (which represent the activation records of methods being invoked). <u>Each time a method is called, a new frame is created and pushed onto the stack. When the method returns, the frame is popped from the stack</u>.

<u>**堆 (Heap)**</u>

- 堆是Java 虚拟机所管理的内存中最大的一块 ;
- Java 堆是被所有线程共享的一块内存区域，在虚拟机启动时创建 ;
- 此内存区域的唯一目的就是<u>存放对象实例</u>，几乎所有的对象实例都在这里分配内存 ;

**元空间 (Metaspace)**

- 存储 常量、静态变量、即时编译器编译后的代码 (比如数据字面量) 等数据，常量池(run-time constant pool) 其实就位于元空间 ;
- 在Java 8及以后的版本中，元空间取代了之前版本中的永久代（PermGen）;
- 与永久代不同，元空间并不在堆内存中，而是使用本地内存（即操作系统管理的内存）, 记住其别名为 non-heap ;



### 内存空间启动参数

有一台16g内存的linux机器, 它完全用于一个java应用实例的部署, 最大化地利用好内存资源

1. **-Xms** 和 **-Xmx**: 为了减少堆内存调整带来的性能开销，通常建议将`-Xms`和`-Xmx`设置为相同的值。例如，如果你的应用需要大量内存，可以将这两个参数设置为<u>总内存的70%-80%</u>。对于16GB的机器，可以设置为`-Xms12g -Xmx12g`。

2. **-Xmn**: 这个参数用于设置 <u>年轻代的大小。年轻代通常包含Eden区和两个Survivor区</u>。年轻代的大小对垃圾回收的性能有很大影响。一般建议 <u>将年轻代设置为堆内存的25%-40%</u>。例如，<u>如果堆内存为12GB，可以将年轻代设置为`-Xmn3g`到`-Xmn5g`</u>。

3. **-Xss**: 这个参数设置 <u>每个线程的栈大小</u>。默认值通常是1M，但在内存有限的情况下，可以适当减小这个值以支持更多的线程。例如，可以设置为`-Xss512k`。

综合以上建议，一个可能的配置如下：
```bash
java -Xms12g -Xmx12g -Xmn4g -Xss512k -jar your-application.jar
```
...
1. `-XX:NewRatio`：设置老年代与新生代的比例。例如，`-XX:NewRatio=2` 表示老年代是新生代的2倍  ===> 如果不想触发频繁full gc，可以通过调大MaxTenuringThreshold来限制 对象 添加 老年代

2. `-XX:SurvivorRatio`：设置Eden区与Survivor区的比例, 例如`-XX:SurvivorRatio=8`, 则表示Eden区为survivor区的8倍 ===> 如果不想触发频繁minor gc，可以通过**调大 新生代空间 实现**（伊甸区、存活区） ;

3. `-XX:+UseSerialGC` / `-XX:+UseParallelGC` / `-XX:+UseConcMarkSweepGC` / `-XX:+UseG1GC`：分别指定使用 串行垃圾收集器、并行垃圾收集器、并发标记清除垃圾收集器和G1垃圾收集器。

4. `-XX:MaxGCPauseMillis`：设置垃圾回收的最大暂停时间目标。JVM会尝试调整堆大小和其他参数以达到这个目标。

5. `-XX:GCTimeRatio`：设置垃圾回收时间与应用程序时间的比例。例如，`-XX:GCTimeRatio=9` 表示垃圾回收时间占总时间的1/(1+9)，即10%。

6. `-XX:+PrintGCDetails` 和 `-XX:+PrintGCTimeStamps`：打印垃圾回收的详细信息和时间戳，有助于分析垃圾回收的性能。



### 选择垃圾回收器

在不同的Java虚拟机（JVM）实现中，默认的垃圾收集器可能会有所不同。截至我知识更新的时间点（2023年），对于Oracle的HotSpot JVM，以下是一些常见的默认垃圾收集器：

1. **客户端模式（Client模式）**：在32位的客户端模式下，默认的垃圾收集器通常是**串行垃圾收集器（Serial GC）**。这是因为串行垃圾收集器简单且高效，适合于单核或者对吞吐量要求不高的桌面应用。
2. **服务器模式（Server模式）**：在64位的服务器模式下，默认的垃圾收集器通常是**并行垃圾收集器（Parallel GC）**，也称为吞吐量收集器。这是因为并行垃圾收集器通过多线程并行执行垃圾收集，可以提高吞吐量，适合于多核处理器和对吞吐量有较高要求的应用。
3. **Java 9及以后的版本**：从Java 9开始，G1垃圾收集器（Garbage-First GC）被设置为默认的垃圾收集器。<u>G1垃圾收集器旨在为具有大内存的多处理器机器提供高吞吐量和低延迟</u>。它通过将堆内存划分为多个区域，并根据各个区域的垃圾回收价值来优先回收，从而实现更可预测的停顿时间。
...
> **Excessive Recursion**: If a method calls itself recursively without a proper base case or with a base case that is never reached, the stack will keep filling up with frames for each recursive call. Eventually, the stack space will be fully consumed, leading to a `StackOverflowError`
>
> **Deeply Nested Call Stack**: Even without recursion, a series of deeply nested method calls can also lead to a `StackOverflowError` if the total depth of the call stack exceeds the available stack space
...
- 用于同一个线程间，多个方法、层次间参数信息传递 —— 比如 **登陆用户的柜员信息、全局业务跟踪号** 一类，如果作为方法参数多层传递，从controller、service到mapper都作为参数传递，那很累赘，不如在请求到达时，通过拦截器，将cookie中的用户信息解析出来、将全剧业务跟踪号生成好放到ThreadLocal中，任何层次用到都可直接从上下文中获取。
...
**注意 :** 阿里编程规范 禁止使用Executors类 创建线程池，而是使用ThreadPoolExecutor类，目的在于 **细化线程池规则，避免资源浪费**  ----> Executors类线程池创建着方便，但 可能导致比如 **工作队列过长 或者 线程数目过多** 的问题，侵占内存，导致OOM（out of memory）
...
- 可自定义TaskDecorator, 以传递主线程ThreadLocal数据到新线程;
- 可避免@Async注解的方法无限启线程 



In Java with Spring Boot, when you annotate a method with `@Async`, it does not directly start a new thread by itself. Instead, it tells the Spring framework to run the annotated method in a thread taken from a thread pool that is managed by Spring's task executor.

By default, if you haven't explicitly defined a `TaskExecutor`, Spring will use a `SimpleAsyncTaskExecutor` to run the `@Async` method. <u>This default task executor does not reuse threads</u>. Instead, it <u>creates a new thread for each method invocation</u>. However, it's typically recommended for production environments to configure a more robust `TaskExecutor` that uses a thread pool for better resource management.



### PoolExcutor对比

TPE: 属于java concurrency API

TPTE: 属于springboot生态



In summary, if you are working within a Spring Boot application and need to manage task execution, you would typically <u>use `ThreadPoolTaskExecutor` for its ease of integration</u> and configuration within the Spring environment. If you are working outside of Spring or need more detailed control over thread management that isn’t provided by the Spring abstraction, you would use `ThreadPoolExecutor`.
...
Java线程池的配置并没有一个固定的“最佳”核心线程数与最大线程数，因为这主要取决于你的应用程序需求、工作负载特性和目标运行环境。不过，确实与CPU的核数有密切关系，通常建议以下面的原则作为出发点进行调整：

1. **核心线程数（corePoolSize）**：这个值通常设置为 **CPU的核心数**。核心线程不会被轻易终止，即使它们处于空闲状态。这样可以减少创建和销毁线程的开销，同时利用好CPU资源。对于CPU密集型任务（如大量计算），设置为核心数或核心数+1是一个常见做法。

2. **最大线程数（maximumPoolSize）**：这个值可以根据应用的具体情况设定，但通常建议不超过可用处理器数量的几倍（比如 **CPU核心数*2**）。这是因为对于 CPU密集型任务，增加超过CPU核心数的线程并不能带来性能提升，反而可能因为上下文切换增加而降低效率。而对于I/O密集型任务，由于线程在等待I/O操作时会被阻塞，可以适当增加最大线程数以充分利用CPU在等待期间处理其他任务。

3. **工作队列（workQueue）**：选择合适的队列类型和大小也很重要。常用的有无界队列（如`LinkedBlockingQueue`）、有界队列（如`ArrayBlockingQueue`）和同步移交队列（如`SynchronousQueue`）。无界队列可能导致内存溢出风险，而有界队列和同步移交队列可以帮助控制任务堆积和系统负载。

总结来说，一个常见的初始配置可能是将核心线程数设为CPU核心数，最大线程数设为CPU核心数的2倍，并使用一个适当大小的有界队列来平衡吞吐量和响应时间。



### CPU密集型 VS I/O密集型

在实际互联网业务中，CPU密集型操作和IO密集型操作区分明显，它们分别对应着不同的任务类型和资源需求。以下是一些典型的例子：

#### CPU密集型操作（CPU-bound）
CPU密集型操作指的是那些主要依赖处理器计算能力的任务，这类任务的特点是程序运行期间大部分时间都在进行计算，而非等待外部输入输出。在互联网业务中，CPU密集型操作的例子包括但不限于：

1. **数据加密与解密**：如HTTPS通信中的SSL/TLS加密解密，以及数据库密码存储的哈希计算。

2. **图像处理与视频编码解码**：如上传图片的即时缩放、滤镜应用，或者视频流的转码处理。

3. **科学计算与算法处理**：复杂的数学运算、数据分析、机器学习模型的训练与预测等。

4. **大数据处理**：如数据挖掘、批量数据处理和分析任务，特别是在没有使用并行计算框架的情况下。

5. **高性能计算**：如数值模拟、大规模并行计算任务

   

#### IO密集型操作（IO-bound）
IO密集型操作则侧重于等待外部设备（如磁盘、网络）的输入输出操作完成，这类任务在执行时，CPU经常处于空闲状态，等待数据的读取或写入。在互联网业务中，IO密集型操作的例子包括：

1. **文件读写操作**：如从磁盘读取日志文件、上传下载文件到云存储。
2. **数据库操作**：尤其是当数据库查询涉及大量数据读写且索引优化不足时，如复杂SQL查询。
3. **网络通信**：HTTP请求响应、WebSocket通信、跨服务API调用等，等待网络数据包的接收和发送。
4. **磁盘I/O**：如网页服务器频繁读取静态资源文件、数据库查询导致的磁盘访问。
5. **外部系统交互**：与缓存系统（如Redis）、消息队列（如RabbitMQ、Kafka）的交互，这些通常涉及网络I/O和潜在的磁盘I/O。

理解任务的性质并据此进行合理的资源分配和架构设计，对于提升互联网服务的性能和效率至关重要。例如，CPU密集型任务可能更适合使用更强大的计算单元或分布式计算，而IO密集型任务则可以通过异步处理、多线程、协程或使用更快的存储解决方案等方式优化。
...
> 在可以配置TaskDecorator的ThreadPoolTaskExecutor中（由spring提供，TreadPoolExecutor由java.concurrent包提供），默认 创建LinkedBlockingQueue形式 的 有界队列
...
- 生成报表, 会使用redis的分布式锁, 根据报表生成周期 制定redis 前缀
...
- 同数组，大小、长度固定，所以 <u>插入、删除操作</u> 会比较麻烦；
...
- HashMap 中元素是没有顺序的；TreeMap中所有元素都是有**某一固定顺序的**；
- HashMap 继承AbstractMap类，是基于 hash表 实现的；
- TreeMap 继承SortedMap类，是基于 红黑树 实现的 ;
...
![Snipaste_2022-08-03_16-41-58](\images\Snipaste_2022-08-03_16-41-58.png)
...
从两个get/put核心方法可以看出 1.8 中对大链表做了优化，修改为 **红黑树** 之后查询效率直接提高到了 O(logn)。但是 HashMap 原有的问题也都存在，比如在并发场景下使用时容易出现死循环 :
...
接口MenuService有多个实现类, 如何 实现 指定注入 ? 此时如果直接 `private MenuService menuService` , 加

此时，若希望生成的bean为DefaultMenuService对象，相关策略包括 : 

​                ● 给DefaultMenuService的Service注解添加默认value参数@Service（“menuService”）;

​                ● 在注入注解autowired下方加Qualifier注解，值为“**d**efaultMenuService” ;

​                ● 去掉autowired注解，添加Resource注解（name=“DefaultMenuService”）
...
> - 允许自动配置: xml手动为每个框架进行配置, 繁琐, 开启自动配置后, 每个框架都采取默认配置, 除非在配置文件里specify ;
> - 万物皆可为bean —— 配置也可以用bean进行
...
**@Autowired**：用于标记Spring将要解析和注入的依赖项。此注解可以作用在**构造函数、字段和setter方法**上。默认情况下必须要求 依赖对象 必须存在，如果要允许null 值，可以设置它的required属性为false，如@Autowired（required=false）

**@Qualifier**：经常与@Autowired搭配使用，默认值属性为 优先装配的实现类名

**@Resource**：优选，Java原生注解，**优先按照 属性名/成员变量名 装配，若不存在同名beanId，则按类型装配**，都不存在则报错

> A优先按 类型 装配, 如果发现MenuService下有多个实现类, 它会报错, 此时需要结合Qualifier指定实现类;
>
> R优先按 名字 装配, more exception free, 比如 命名为 menuService1, 就会找名为 MenuService1实现类, 没有就会找 MenuService 实现类, 只要有就注入



**@Primary：**当系统中需要配置多个具有相同数据类型的bean时，@Primary可以定义这些Bean的优先级。比如多个实现类serviceImpl，有一个实现类注解加@Primary则优先实现该类。
...
### Integer.MAX_VALUE + 1 等于多少? 

在Java中，`Integer.MAX_VALUE`代表`int`类型能表示的最大数值，其值为`2^31 - 1`，即`2147483647`。当你对`Integer.MAX_VALUE`加1时，会发生整数溢出，结果会回绕到`int`类型能表示的最小值，即`-2^31`，也就是`-2147483648`。这种行为在计算机科学中称为"环绕"（wrap around）或"溢出"（overflow）。

所以，`Integer.MAX_VALUE + 1`等于`-2147483648`。
